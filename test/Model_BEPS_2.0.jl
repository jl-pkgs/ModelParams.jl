...
# 水力参数 (Campbell retention model)
@bounds @units @with_kw mutable struct Campbell{FT<:AbstractFloat}
    θ_sat::FT = FT(0.45) | (0.25, 0.70) | "m3 m-3"   # volumetric saturation
    ψ_sat::FT = FT(-0.5) | (-2.0, -0.01) | "m"        # matric potential at saturation
    Ksat::FT = FT(5.0) | (0.01, 50.0) | "cm h-1"      # saturated hydraulic conductivity
    b::FT = FT(5.0) | (1.5, 15.0) | "-"               # Campbell b parameter
    θ_fc::FT = FT(0.30) | (0.10, 0.45) | "m3 m-3"    # field capacity
    θ_vwp::FT = FT(0.10) | (0.02, 0.30) | "m3 m-3"   # wilting point
end
...
@with_kw mutable struct ParamSoil{FT<:AbstractFloat}
    hydraulic::Campbell{FT} = Campbell{FT}()
