...
export default_hydraulic, default_kv_profile
export _sync_ksat!
export ParamThermal, ParamThermalLayers, ThermalProfile


abstract type AbstractRetention{T<:Real} end
abstract type AbstractRetentionLayers{FT,S} <: AbstractLayers{FT,S} end  # 持水模型层基类，新模型继承即可
...
@with_kw mutable struct HydraulicProfile{FT<:AbstractFloat,P<:AbstractRetention{FT}}
    N::Int = 5
    profile::AbstractRetentionLayers{FT} = CampbellLayers{FT,N}()  # SoA: 持水模型层参数
...
function default_kv_profile(retention::AbstractRetentionLayers{FT}) where {FT<:AbstractFloat}
    N = length(retention)
    layers = Vector(retention)
...
@with_kw mutable struct ThermalProfile{FT<:AbstractFloat}
    N::Int = 5
    profile::ParamThermalLayers{FT} = ParamThermalLayers{FT,N}()
    layers::Vector{ParamThermal{FT}} = Vector(profile)
end
